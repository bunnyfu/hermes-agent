"""Attachment corruption regression: the t_2ae14d55 incident (2026-09-05).

On 2026-09-05 a scout worker attached a 16,986 B audit report to its task on
board s2-engram via ``kanban_attach`` but passed a hallucinated placeholder as
``content_base64`` — the string ``L2FuZHJvaWQvLi4v``, which validly decodes to
...
a lie (finding t_5a72b79b, audits t_553fe667 + t_e06fe226).
...
def test_attach_rejects_incident_payload(worker_env):
    """The exact t_2ae14d55 invocation shape must be a clean rejection.
